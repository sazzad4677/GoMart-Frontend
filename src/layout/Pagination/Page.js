...
const Page = ({ ...props }) => {
    let { resultPerPage, keyword, currentPage, setCurrentPage, totalProductsCount, filteredProductsCount, sortType } = props
    // result count of total product count per page
    let counts = ((keyword || sortType) ? filteredProductsCount : totalProductsCount) || 0
    const totalPageCount = Math.round(counts / resultPerPage)
    const [start, setStart] = useState(parseInt((currentPage - 1) * resultPerPage) + 1)
    ...
    return (
        <div className="bg-white px-4 py-3 flex items-center justify-between sm:px-6 ">
            <div className="pagination justify-self-center" role="navigation" aria-label="pagination">
                <Pagination
