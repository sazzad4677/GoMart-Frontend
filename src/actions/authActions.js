...
import {
    ...
    LOAD_USER_REQUEST, LOAD_USER_SUCCESS, LOAD_USER_FAILED,
    LOGOUT_USER_SUCCESS, LOGOUT_USER_FAILED,
    CLEAR_ERRORS
...
export const login = (email, password, remember) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post('/api/v1/login', { email, password, remember }, config)
        dispatch({
            type: LOGIN_SUCCESS,
            payload: data.user
        })
